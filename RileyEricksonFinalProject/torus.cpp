...
void fillTorVertexArray(Vertex torVertices[(TOR_LONGS + 1) * (TOR_LATS + 1)])
{
   ...
   for (j = 0; j <= TOR_LATS; j++)
      for (i = 0; i <= TOR_LONGS; i++)
      {
		 ...
		 torVertices[k].normal.x = cos( (-1 + 2 * (float)i / TOR_LONGS) * PI );
         torVertices[k].normal.y = sin( (-1 + 2 * (float)i / TOR_LONGS) * PI );
		 torVertices[k].normal.z = (-1 * TOR_INRAD * sin( (-1 + 2*(float)j/TOR_LATS) * PI ));
		 k++;
	  }
}
